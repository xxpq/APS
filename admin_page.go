...
<body>
  ...
  <div class="mobile-sidebar" id="mobile-sidebar">
    ...
    <a class="mobile-nav-item auth-required" href="#" data-tab="tab-rate-limits">流控</a>
  ...
  <header class="bx--header" role="banner" aria-label="APS Admin">
    ...
    <nav class="bx--header__nav" aria-label="APS 管理面板">
      <ul class="bx--header__menu-bar">
        ...
        <li class="auth-required"><a class="bx--header__menu-item" href="#" data-tab="tab-rate-limits">流控</a></li>
  ...
  <main class="container">
    ...
    <div data-modal id="user-add-modal" class="bx--modal" role="dialog" aria-labelledby="user-add-title">
      <div class="bx--modal-container">
        ...
        <div class="bx--modal-content">
          ...
          <div class="bx--form-item mt-1">
            <input class="bx--toggle-input bx--toggle-input--small" id="add-user-proxy" type="checkbox">
            <label class="bx--toggle-input__label" for="add-user-proxy">
              代理权限
              <span class="bx--toggle__switch">
                <span class="bx--toggle__text--off" aria-hidden="true">否</span>
                <span class="bx--toggle__text--on" aria-hidden="true">是</span>
    ...
    <div data-modal id="user-edit-modal" class="bx--modal" role="dialog" aria-labelledby="user-edit-title">
      <div class="bx--modal-container">
        ...
        <div class="bx--modal-content">
          ...
          <div class="bx--form-item mt-1">
            <input class="bx--toggle-input bx--toggle-input--small" id="edit-user-proxy" type="checkbox">
            <label class="bx--toggle-input__label" for="edit-user-proxy">
              代理权限
              <span class="bx--toggle__switch">
                <span class="bx--toggle__text--off" aria-hidden="true">否</span>
                <span class="bx--toggle__text--on" aria-hidden="true">是</span>
          ...
          <div class="bx--form-item mt-1"><label class="bx--label">访问令牌（可选）</label><input id="edit-user-token" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">流控规则 (可选)</label><input id="edit-user-rate-limit-rules" type="text" class="bx--text-input" placeholder="rule1, rule2"><div class="bx--form__helper-text">多个规则用逗号分隔</div></div>
    ...
    <section id="tab-servers" class="bx--tab-content hidden" role="tabpanel" aria-labelledby="服务">
      <div class="bx--tile">
        ...
        <div class="table-wrap">
          <table class="bx--data-table carbon-table">
            <thead><tr><th>服务名称</th><th>监听端口</th><th>服务类型</th><th>SSL证书</th><th>安全策略</th><th>操作</th></tr></thead>
    ...
    <!-- 流控规则管理 -->
    <section id="tab-rate-limits" class="bx--tab-content hidden" role="tabpanel" aria-labelledby="流控">
      <div class="bx--tile">
        <div class="flex mb-2">
          <button id="btn-rate-limits-load" class="bx--btn bx--btn--secondary">加载规则</button>
          <button id="btn-rate-limits-add" class="bx--btn bx--btn--primary">新增规则</button>
        </div>
        <div class="table-wrap">
          <table class="bx--data-table carbon-table">
            <thead><tr><th>规则名称</th><th>目标类型</th><th>指标</th><th>动作</th><th>操作</th></tr></thead>
            <tbody id="rate-limits-tbody"></tbody>
          </table>
        </div>
        <div id="rate-limits-msg" class="mt-2"></div>
      </div>
    </section>

    <!-- 新增流控规则对话框 -->
    <div data-modal id="rate-limit-rule-add-modal" class="bx--modal" role="dialog">
      <div class="bx--modal-container" style="max-width: 800px;">
        <div class="bx--modal-header">
          <p class="bx--modal-header__heading">新增流控规则</p>
          <button class="bx--modal-close" type="button" data-modal-close aria-label="关闭">
            <svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg>
          </button>
        </div>
        <div class="bx--modal-content">
          <div class="bx--form-item"><label class="bx--label">规则名称 *</label><input id="add-rate-limit-name" type="text" class="bx--text-input"></div>
          <div class="bx--form-item mt-1">
            <label class="bx--label">目标类型</label>
            <select id="add-rate-limit-target" class="bx--select">
          ...
          <div class="bx--form-item mt-2">
            <label class="bx--label">指标配置 (每行一个: 类型,窗口秒数,阈值)</label>
            <div class="bx--form__helper-text">类型: request_count(请求数), concurrent_requests(并发数), traffic_volume(流量字节), transfer_rate(速率字节/秒)</div>
            <textarea id="add-rate-limit-metrics" class="bx--text-input" rows="4" placeholder='request_count,60,100&#10;concurrent_requests,0,10'></textarea>
          </div>

          <div class="bx--form-item mt-2">
            <label class="bx--label">动作配置 (每行一个: 类型,参数)</label>
            <div class="bx--form__helper-text">类型: ban(封禁秒数), queue(最大等待秒数), redirect(跳转URL)</div>
            <textarea id="add-rate-limit-actions" class="bx--text-input" rows="3" placeholder='ban,300&#10;queue,10'></textarea>
          </div>
        </div>
        <div class="bx--modal-footer">
          <button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button>
          <button class="bx--btn bx--btn--primary" type="button" id="confirm-add-rate-limit">确认新增</button>
        </div>
      </div>
    </div>

    <!-- 编辑流控规则对话框 -->
    <div data-modal id="rate-limit-rule-edit-modal" class="bx--modal" role="dialog">
      <div class="bx--modal-container" style="max-width: 800px;">
        <div class="bx--modal-header">
          <p class="bx--modal-header__heading">编辑流控规则</p>
          <button class="bx--modal-close" type="button" data-modal-close aria-label="关闭">
            <svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg>
          </button>
        </div>
        <div class="bx--modal-content">
          <input type="hidden" id="edit-rate-limit-original-name">
          <div class="bx--form-item"><label class="bx--label">规则名称 *</label><input id="edit-rate-limit-name" type="text" class="bx--text-input"></div>
          <div class="bx--form-item mt-1">
            <label class="bx--label">目标类型</label>
            <select id="edit-rate-limit-target" class="bx--select">
              <option value="ip">IP地址</option>
              <option value="token">访问令牌</option>
            </select>
          </div>
          ...
          <div class="bx--form-item mt-2">
            <label class="bx--label">指标配置 (每行一个: 类型,窗口秒数,阈值)</label>
            <textarea id="edit-rate-limit-metrics" class="bx--text-input" rows="4"></textarea>
          </div>

          <div class="bx--form-item mt-2">
            <label class="bx--label">动作配置 (每行一个: 类型,参数)</label>
            <textarea id="edit-rate-limit-actions" class="bx--text-input" rows="3"></textarea>
          </div>
        </div>
        <div class="bx--modal-footer">
          <button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button>
          <button class="bx--btn bx--btn--primary" type="button" id="confirm-edit-rate-limit">确认保存</button>
        </div>
      </div>
    </div>
    ...
    <div data-modal id="server-add-modal" class="bx--modal"><div class="bx--modal-container"><div class="bx--modal-header"><p class="bx--modal-header__heading">新增服务</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><div class="bx--form-item"><label class="bx--label">服务名称 *</label><input id="add-server-name" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">监听端口 *</label><input id="add-server-port" type="number" class="bx--text-input" placeholder="8080"></div><div class="bx--form-item mt-1"><label class="bx--label">SSL证书</label><select id="add-server-cert" class="bx--select"><option value="">无</option><option value="auto">Auto</option><option value="acme">ACME</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">安全策略</label><select id="add-server-firewall" class="bx--select"><option value="">无</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">流控规则 (可选)</label><input id="add-server-rate-limit-rules" type="text" class="bx--text-input" placeholder="rule1, rule2"><div class="bx--form__helper-text">多个规则用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">服务类型</label><select id="add-server-type" class="bx--select"><option value="1">1 - TCP</option><option value="2" selected>2 - HTTP</option><option value="3">3 - UDP</option><option value="4">4 - TCP+UDP</option><option value="5">5 - HTTP+UDP</option></select></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="add-server-public" type="checkbox" class="bx--checkbox"><label for="add-server-public" class="bx--checkbox-label"><span class="bx--checkbox-label-text">公共服务</span></label></div></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="add-server-panel" type="checkbox" class="bx--checkbox"><label for="add-server-panel" class="bx--checkbox-label"><span class="bx--checkbox-label-text">管理面板</span></label></div></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="add-server-proxy" type="checkbox" class="bx--checkbox"><label for="add-server-proxy" class="bx--checkbox-label"><span class="bx--checkbox-label-text">代理服务</span></label></div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="add-server-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="add-server-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-add-server">确认新增</button></div></div></div>
    <div data-modal id="server-edit-modal" class="bx--modal"><div class="bx--modal-container"><div class="bx--modal-header"><p class="bx--modal-header__heading">编辑服务</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><input type="hidden" id="edit-server-original-name"><div class="bx--form-item"><label class="bx--label">服务名称 *</label><input id="edit-server-name" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">监听端口 *</label><input id="edit-server-port" type="number" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">SSL证书</label><select id="edit-server-cert" class="bx--select"><option value="">无</option><option value="auto">Auto</option><option value="acme">ACME</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">安全策略</label><select id="edit-server-firewall" class="bx--select"><option value="">无</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">流控规则 (可选)</label><input id="edit-server-rate-limit-rules" type="text" class="bx--text-input" placeholder="rule1, rule2"><div class="bx--form__helper-text">多个规则用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">服务类型</label><select id="edit-server-type" class="bx--select"><option value="1">1 - TCP</option><option value="2">2 - HTTP</option><option value="3">3 - UDP</option><option value="4">4 - TCP+UDP</option><option value="5">5 - HTTP+UDP</option></select></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="edit-server-public" type="checkbox" class="bx--checkbox"><label for="edit-server-public" class="bx--checkbox-label"><span class="bx--checkbox-label-text">公共服务</span></label></div></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="edit-server-panel" type="checkbox" class="bx--checkbox"><label for="edit-server-panel" class="bx--checkbox-label"><span class="bx--checkbox-label-text">管理面板</span></label></div></div><div class="bx--form-item mt-1"><div class="bx--checkbox-wrapper"><input id="edit-server-proxy" type="checkbox" class="bx--checkbox"><label for="edit-server-proxy" class="bx--checkbox-label"><span class="bx--checkbox-label-text">代理服务</span></label></div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="edit-server-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="edit-server-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-edit-server">确认保存</button></div></div></div>



    <!-- Rule Add/Edit Modals -->
    <div data-modal id="rule-add-modal" class="bx--modal"><div class="bx--modal-container" style="max-width: 600px;"><div class="bx--modal-header"><p class="bx--modal-header__heading">新增路由规则</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><div class="bx--form-item"><label class="bx--label">源路径 (From) *</label><textarea id="add-rule-from" class="bx--text-input" rows="2" placeholder="/api/*&#10;或多行URL"></textarea><div class="bx--form__helper-text">支持单个URL或多个URL（每行一个）</div></div><div class="bx--form-item mt-1"><label class="bx--label">目标路径 (To) *</label><input id="add-rule-to" type="text" class="bx--text-input" placeholder="http://backend/*"></div><div class="bx--form-item mt-1"><label class="bx--label">Via Endpoints (可选)</label><input id="add-rule-via-endpoints" type="text" class="bx--text-input" placeholder="endpoint_name"><div class="bx--form__helper-text">通过指定的端点转发</div></div><div class="bx--form-item mt-1"><label class="bx--label">认证绑定 (可选)</label><select id="add-rule-auth-provider" class="bx--select"><option value="">无</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">流控规则 (可选)</label><input id="add-rule-rate-limit-rules" type="text" class="bx--text-input" placeholder="rule1, rule2"><div class="bx--form__helper-text">多个规则用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">服务器列表 (可选)</label><input id="add-rule-servers" type="text" class="bx--text-input" placeholder="server1, server2"><div class="bx--form__helper-text">多个服务器用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="add-rule-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="add-rule-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-add-rule">确认新增</button></div></div></div>
    <div data-modal id="rule-edit-modal" class="bx--modal"><div class="bx--modal-container" style="max-width: 600px;"><div class="bx--modal-header"><p class="bx--modal-header__heading">编辑路由规则</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><input type="hidden" id="edit-rule-index"><div class="bx--form-item"><label class="bx--label">源路径 (From) *</label><textarea id="edit-rule-from" class="bx--text-input" rows="2"></textarea><div class="bx--form__helper-text">支持单个URL或多个URL（每行一个）</div></div><div class="bx--form-item mt-1"><label class="bx--label">目标路径 (To) *</label><input id="edit-rule-to" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">Via Endpoints (可选)</label><input id="edit-rule-via-endpoints" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">认证绑定 (可选)</label><select id="edit-rule-auth-provider" class="bx--select"><option value="">无</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">流控规则 (可选)</label><input id="edit-rule-rate-limit-rules" type="text" class="bx--text-input" placeholder="rule1, rule2"><div class="bx--form__helper-text">多个规则用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">服务器列表 (可选)</label><input id="edit-rule-servers" type="text" class="bx--text-input"><div class="bx--form__helper-text">多个服务器用逗号分隔</div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="edit-rule-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="edit-rule-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-edit-rule">确认保存</button></div></div></div>



    <!-- Rate Limit Rule Add/Edit Modals -->
    <div data-modal id="rate-limit-add-modal" class="bx--modal"><div class="bx--modal-container"><div class="bx--modal-header"><p class="bx--modal-header__heading">新增流控规则</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><div class="bx--form-item"><label class="bx--label">规则名称 *</label><input id="add-rl-name" type="text" class="bx--text-input" placeholder="例如: global-limit"></div><div class="bx--form-item mt-1"><label class="bx--label">目标类型</label><select id="add-rl-target" class="bx--select"><option value="ip">IP</option><option value="token">Token</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">指标配置 (JSON)</label><textarea id="add-rl-metrics" class="bx--text-area" rows="4" placeholder='[{"type":"request_count","window":60,"threshold":100}]'></textarea><div class="bx--form-requirement">支持: request_count, concurrent_requests, traffic_volume, transfer_rate</div></div><div class="bx--form-item mt-1"><label class="bx--label">动作配置 (JSON)</label><textarea id="add-rl-actions" class="bx--text-area" rows="4" placeholder='[{"type":"ban","duration":300}]'></textarea><div class="bx--form-requirement">支持: ban, queue, redirect</div></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" onclick="confirmAddRateLimit()">保存</button></div></div></div>
    <div data-modal id="rate-limit-edit-modal" class="bx--modal"><div class="bx--modal-container"><div class="bx--modal-header"><p class="bx--modal-header__heading">编辑流控规则</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><input type="hidden" id="edit-rl-original-name"><div class="bx--form-item"><label class="bx--label">规则名称 *</label><input id="edit-rl-name" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">目标类型</label><select id="edit-rl-target" class="bx--select"><option value="ip">IP</option><option value="token">Token</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">指标配置 (JSON)</label><textarea id="edit-rl-metrics" class="bx--text-area" rows="4"></textarea><div class="bx--form-requirement">支持: request_count, concurrent_requests, traffic_volume, transfer_rate</div></div><div class="bx--form-item mt-1"><label class="bx--label">动作配置 (JSON)</label><textarea id="edit-rl-actions" class="bx--text-area" rows="4"></textarea><div class="bx--form-requirement">支持: ban, queue, redirect</div></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" onclick="confirmEditRateLimit()">保存</button></div></div></div>

    <!-- Firewall Add/Edit Modals -->
    <div data-modal id="firewall-add-modal" class="bx--modal"><div class="bx--modal-container" style="max-width: 700px;"><div class="bx--modal-header"><p class="bx--modal-header__heading">新增防火墙策略</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><div class="bx--form-item"><label class="bx--label">策略名称 *</label><input id="add-firewall-name" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">允许网络（每行一个CIDR或IP）</label><textarea id="add-firewall-allow" class="bx--text-input" rows="4" placeholder="192.168.1.0/24&#10;10.0.0.1"></textarea></div><div class="bx--form-item mt-1"><label class="bx--label">拒绝网络（每行一个CIDR或IP）</label><textarea id="add-firewall-block" class="bx--text-input" rows="4" placeholder="0.0.0.0/0"></textarea></div><div class="bx--form-item mt-1"><label class="bx--label">允许地区（格式: CN 或 US-California，回车添加）</label><input id="add-firewall-allow-regions-input" type="text" class="bx--text-input" placeholder="输入国家代码或 国家-地区，回车添加"><div id="add-firewall-allow-regions-tags" class="tag-container mt-1"></div></div><div class="bx--form-item mt-1"><label class="bx--label">拒绝地区（格式: CN 或 US-California，回车添加）</label><input id="add-firewall-block-regions-input" type="text" class="bx--text-input" placeholder="输入国家代码或 国家-地区，回车添加"><div id="add-firewall-block-regions-tags" class="tag-container mt-1"></div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="add-firewall-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="add-firewall-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-add-firewall">确认新增</button></div></div></div>
    <div data-modal id="firewall-edit-modal" class="bx--modal"><div class="bx--modal-container" style="max-width: 700px;"><div class="bx--modal-header"><p class="bx--modal-header__heading">编辑防火墙策略</p><button class="bx--modal-close" type="button" data-modal-close aria-label="关闭"><svg class="bx--modal-close__icon" width="16" height="16" viewBox="0 0 16 16"><path d="M12 4.7L11.3 4 8 7.3 4.7 4 4 4.7 7.3 8 4 11.3 4.7 12 8 8.7 11.3 12 12 11.3 8.7 8z"/></svg></button></div><div class="bx--modal-content"><input type="hidden" id="edit-firewall-original-name"><div class="bx--form-item"><label class="bx--label">策略名称 *</label><input id="edit-firewall-name" type="text" class="bx--text-input"></div><div class="bx--form-item mt-1"><label class="bx--label">允许网络（每行一个CIDR或IP）</label><textarea id="edit-firewall-allow" class="bx--text-input" rows="4"></textarea></div><div class="bx--form-item mt-1"><label class="bx--label">拒绝网络（每行一个CIDR或IP）</label><textarea id="edit-firewall-block" class="bx--text-input" rows="4"></textarea></div><div class="bx--form-item mt-1"><label class="bx--label">允许地区（格式: CN 或 US-California，回车添加）</label><input id="edit-firewall-allow-regions-input" type="text" class="bx--text-input" placeholder="输入国家代码或 国家-地区，回车添加"><div id="edit-firewall-allow-regions-tags" class="tag-container mt-1"></div></div><div class="bx--form-item mt-1"><label class="bx--label">拒绝地区（格式: CN 或 US-California，回车添加）</label><input id="edit-firewall-block-regions-input" type="text" class="bx--text-input" placeholder="输入国家代码或 国家-地区，回车添加"><div id="edit-firewall-block-regions-tags" class="tag-container mt-1"></div></div><div class="bx--form-item mt-1"><label class="bx--label">日志等级</label><select id="edit-firewall-log-level" class="bx--select"><option value="">继承默认</option><option value="0">0 - 不记录</option><option value="1">1 - 基础信息</option><option value="2">2 - 完整请求</option></select></div><div class="bx--form-item mt-1"><label class="bx--label">日志保留 (小时)</label><input id="edit-firewall-log-retention" type="number" class="bx--text-input" placeholder="继承默认"></div></div><div class="bx--modal-footer"><button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button><button class="bx--btn bx--btn--primary" type="button" id="confirm-edit-firewall">确认保存</button></div></div></div>
