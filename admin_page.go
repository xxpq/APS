      ...
      <div class="bx--modal-container" style="max-width: 800px;">
        ...
        <div class="bx--modal-content">
          ...
          <div class="bx--form-item mt-2">
            <label class="bx--label">指标配置 (每行一个: 类型,窗口秒数,阈值)</label>
            <div class="bx--form__helper-text">类型: request_count(请求数), concurrent_requests(并发数), traffic_volume(流量字节), transfer_rate(速率字节/秒)</div>
            <textarea id="edit-rate-limit-metrics" class="bx--text-input" rows="4" placeholder='request_count,60,100&#10;concurrent_requests,0,10'></textarea>
          </div>

          <div class="bx--form-item mt-2">
            <label class="bx--label">动作配置 (每行一个: 类型,参数)</label>
            <div class="bx--form__helper-text">类型: ban(封禁秒数), queue(最大等待秒数), redirect(跳转URL)</div>
            <textarea id="edit-rate-limit-actions" class="bx--text-input" rows="3" placeholder='ban,300&#10;queue,10'></textarea>
          </div>
        </div>
        <div class="bx--modal-footer">
          <button class="bx--btn bx--btn--secondary" type="button" data-modal-close>取消</button>
