...
type StaticCacheConfig struct {
	...
	MaxFileSizeMB      int             `json:"max_file_size_mb,omitempty"`       // 最大缓存文件大小(MB)，默认 50MB  (向后兼容)
	SizeLimit          *CacheSizeLimit `json:"size_limit,omitempty"`             // 详细大小限制配置
...
// CacheSizeLimit 缓存大小限制配置
type CacheSizeLimit struct {
	Mem  string `json:"mem,omitempty"`  // 内存缓存限制，如 "10m", "100m"
